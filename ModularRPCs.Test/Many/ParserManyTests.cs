        ...
        public static unsafe void TestManyParserBytes<T>(T[] values, IArrayBinaryTypeParser<T> parser, Func<T, T, bool> equality = null)
        {
            ...
            fixed (byte* bufferSrc = new byte[maxSize])
            {
                for (bool useObjToWrite = false; ; useObjToWrite = true)
                {
                    ...
                    List<Type> types = new() { typeof(T[]), typeof(IList<T>), typeof(IReadOnlyList<T>) };
                    if (bitArrayParser != null)
                    ...
                    for (int i = 0; i < arrCt; ++i)
                    {
                        ...
                        List<T> outputList = new List<T>(new T[size]);
                    ...
                    for (int i = 0; i < arrCt; ++i)
                    {
                        List<T> outputList = new List<T>(new T[values.Length]);
                    ...
                    for (int i = 0; i < arrCt; ++i)
                    {
                        ...
                        ListWrapper<T> outputList = new ListWrapper<T>(new List<T>(new T[size]));
                        int readCt = parser.ReadObject(buffer + pos, maxSize - (uint)pos, outputList, out bytesRead, measuredCount: size, hasReadLength: true, setInsteadOfAdding: true);
                        Assert.That(readCt, Is.EqualTo(outputList.Count));
                        pos += bytesRead;
                        Assert.That(outputList, IsEqualToCustom(values));
                    ...
                    for (int i = 0; i < arrCt; ++i)
                    {
                        ListWrapper<T> outputList = new ListWrapper<T>(new List<T>(new T[values.Length]));
                        int readCt = parser.ReadObject(buffer + pos, maxSize - (uint)pos, outputList, out int bytesRead, hasReadLength: false, setInsteadOfAdding: true);
                        Assert.That(readCt, Is.EqualTo(outputList.Count));
                        pos += bytesRead;
                        Assert.That(outputList, IsEqualToCustom(values));
                    ...
                    if (values.Length != 0)
                    {
                        ...
                        for (int i = 0; i < arrCt; ++i)
                        {
                            ...
                            List<T> outputList = new List<T>(new T[size - 1]);
                            int readCt = parser.ReadObject(buffer + pos, maxSize - (uint)pos, outputList, out bytesRead, measuredCount: size, hasReadLength: true, setInsteadOfAdding: true);
                            Assert.That(readCt, Is.EqualTo(outputList.Count));
                            pos += bytesRead;
                            Assert.That(outputList, IsEqualToCustom(values));
                        ...
                        for (int i = 0; i < arrCt; ++i)
                        {
                            List<T> outputList = new List<T>(new T[values.Length - 1]);
                            int readCt = parser.ReadObject(buffer + pos, maxSize - (uint)pos, outputList, out int bytesRead, hasReadLength: false, setInsteadOfAdding: true);
                            Assert.That(readCt, Is.EqualTo(outputList.Count));
                            pos += bytesRead;
                            Assert.That(outputList, IsEqualToCustom(values));
                        ...
                        for (int i = 0; i < arrCt; ++i)
                        {
                            ...
                            ListWrapper<T> outputList = new ListWrapper<T>(new List<T>(new T[size - 1]));
                            int readCt = parser.ReadObject(buffer + pos, maxSize - (uint)pos, outputList, out bytesRead, measuredCount: size, hasReadLength: true, setInsteadOfAdding: true);
                            Assert.That(readCt, Is.EqualTo(outputList.Count));
                            pos += bytesRead;
                            Assert.That(outputList, IsEqualToCustom(values));
                        ...
                        for (int i = 0; i < arrCt; ++i)
                        {
                            ListWrapper<T> outputList = new ListWrapper<T>(new List<T>(new T[values.Length - 1]));
                            int readCt = parser.ReadObject(buffer + pos, maxSize - (uint)pos, outputList, out int bytesRead, hasReadLength: false, setInsteadOfAdding: true);
                            Assert.That(readCt, Is.EqualTo(outputList.Count));
                            pos += bytesRead;
                            Assert.That(outputList, IsEqualToCustom(values));
