...
public class UInt128Parser : BinaryTypeParser<UInt128>
{
    public override bool IsVariableSize => false;
    public override int MinimumSize => 16;
    public override unsafe int WriteObject(UInt128 value, byte* bytes, uint maxSize)
    {
        if (maxSize < 16)
            throw new RpcOverflowException(string.Format(Properties.Exceptions.RpcOverflowExceptionIBinaryTypeParser, nameof(UInt128Parser))) { ErrorCode = 1 };
    ...
    public override int WriteObject(UInt128 value, Stream stream)
    {
    ...
    public override unsafe UInt128 ReadObject(byte* bytes, uint maxSize, out int bytesRead)
    {
        if (maxSize < 16)
            throw new RpcParseException(string.Format(Properties.Exceptions.RpcParseExceptionBufferRunOutIBinaryTypeParser, nameof(UInt128Parser))) { ErrorCode = 1 };

        UInt128 value;

        if (BitConverter.IsLittleEndian)
        {
            value = Unsafe.ReadUnaligned<UInt128>(bytes);
        }
        else
        {
    ...
    public override UInt128 ReadObject(Stream stream, out int bytesRead)
    {
        Span<byte> span = stackalloc byte[16];
        ...
        if (BitConverter.IsLittleEndian)
        {
            value = Unsafe.ReadUnaligned<UInt128>(ref span[0]);
        }
        else
        {
    ...
    public class Many(SerializationConfiguration config) : UnmanagedValueTypeBinaryArrayTypeParser<UInt128>(config);
}
#endif