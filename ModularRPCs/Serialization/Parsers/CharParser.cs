...
public class CharParser : BinaryTypeParser<char>
{
    public override bool IsVariableSize => false;
    public override int MinimumSize => 2;
    public override unsafe int WriteObject(char value, byte* bytes, uint maxSize)
    {
        if (maxSize < 2)
            throw new RpcOverflowException(string.Format(Properties.Exceptions.RpcOverflowExceptionIBinaryTypeParser, nameof(CharParser))) { ErrorCode = 1 };
    ...
    public override int WriteObject(char value, Stream stream)
    {
    ...
    public override char ReadObject(Stream stream, out int bytesRead)
    {
        int value;
#if NETSTANDARD && !NETSTANDARD2_1_OR_GREATER || NETFRAMEWORK
        byte[] span = DefaultSerializer.ArrayPool.Rent(2);
