...
public abstract class BinaryTypeParser<T> : IBinaryTypeParser<T>
{
    ...
    unsafe object IBinaryTypeParser.ReadObject(byte* bytes, uint maxSize, out int bytesRead)
    {
        return ReadObject(bytes, maxSize, out bytesRead)!;
    }
    object IBinaryTypeParser.ReadObject(Stream stream, out int bytesRead)
    {
